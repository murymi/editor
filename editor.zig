...
pub const Editor = struct {
    ...
    const Row = struct {
        ...
        fn init(alloc: Allocator) @This() {
            return @This(){ .index = 0, .characters = vec(u8).init(alloc) };
        }
    ...
    pub fn init(allocator: Allocator) Self {
        return Self{ .rows = vec(Row).init(allocator), .cursor_pos = .{ .col = 0, .row = 0 }, .allocator = allocator, .filename = "" };
    }
    ...
    pub fn delchar(self: *Self) !void {
        ...
        if (self.cursor_pos.col == 0) {
            const cp = if (self.rows.items[self.cursor_pos.row - 1].characters.items.len > 0)
                self.rows.items[self.cursor_pos.row - 1].characters.items.len
            else
                0;
            try self.rows.items[self.cursor_pos.row - 1].characters.appendSlice(self.rows.items[self.cursor_pos.row].characters.items[0..]);
    ...
    pub fn addfile(self: *Self, f: std.fs.File, filename:[]const u8) !void {
        const buf = try f.readToEndAlloc(self.allocator, std.math.maxInt(u16));
        for (buf) |c| {
            try self.addchar(c);
        }
        ...
        if (self.rows.items.len > self.cursor_pos.row + 1) {
            ...
            if (self.cursor_pos.col >= self.rows.items[self.cursor_pos.row].characters.items.len) {
                self.cursor_pos.col = if (self.rows.items[self.cursor_pos.row].characters.items.len > 0)
                    self.rows.items[self.cursor_pos.row].characters.items.len
                else
                    0;
            }
        }
